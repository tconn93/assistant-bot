...
current_model = config['GPT_MODEL']
...
async def on_ready():
    ...
    print()
    print()
    print(f"\033[1;38;5;46mCurrent model: {current_model}\033[0m")
...
@bot.hybrid_command(name="askgrok", description="Ask Grok a question")
async def askgrok(ctx, prompt: str):
    await ctx.defer()
    response = await generate_grok_response(prompt=prompt)
    for chunk in split_response(response):
